...
get_run_res <- function(k_test=0,vtos,SD=0,mtx=beta_matrix_no_interv, pv){
...
create_country_tbl <- function(k_range_percent,strat_ls){
  ...
  for (strat in strat_ls){
    ...
    for (SD in strat$SD_ls){
      ...
      for (pv in pv_list){
        write_lines(paste('|----> max % vaccinated: ',pv,sep=''), paste(JOB_ID,'log.txt',sep='_'), append = T)
        for (k_percent in k_range_percent){
          ...
          k_res <- NULL
          k_res <- get_run_res(k_test=k, vtos=vto, SD=0, mtx=m, pv = pv) # The SD here refers to the external forcing function. the real SD is given directly in the contact matrix
          k_res$k <- k
