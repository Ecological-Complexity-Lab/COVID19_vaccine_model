...
full_model <- function (t, x, beta_matrix, k, pv, vto,...){
  S <- x[sindex]
  E <- x[eindex]
  P <- x[pindex]
  I <- x[iindex]
  ...
  if(stop_vaccination==F){
    ...
    if(sum(V[vtp])>=pv*sum(N_age_groups[vtp])){
      write_lines(paste('[',ceiling(t),']', ' Switching vaccine target group: reached max prop willing to vaccinate. ',sum(V[vtp]),'>=',ceiling(pv*sum(N_age_groups[vtp])),sep=''), paste(JOB_ID,'log.txt',sep='_'), append = T)
      switch_vacc_group <- T
    }
